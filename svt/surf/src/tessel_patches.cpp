...
{ 
    ...
    ALLOCN(probe_centers, PNT, num_tor_verts);
...
{ 
    ...
    /* find the torus points on the end bordering face_atoms[0] in tor_pts[0],
     and those in the center of the torus, in tor_pts[1] */
    ALLOCMAT(tor_pts, 2, num_tor_verts, VertexType);
    find_torus_points(tor_pts, cusp_pts, num_tor_verts, atom_ptr, probe_centers, cir_radius, torus_cusp);
...
void gen_patches_IV(int atom_id, PNT* comp_verts, int num_verts)
{
    ...
    if (k >= guess_num)
    {
        SURFOUT << "Error: gen_patches_IV(): too small guess_num " << guess_num << ", k = " << k << endl;
        return;
    }

    VEC3_ZERO(ave_center);
    for(j = 0; j < k; j++)
        VEC3_ASN_OP(ave_center, +=, sp_points[j].tes_dir);
...
/*--------------------------------------------------------------------------------
 find_order accepts PNTs and returns 0 if in going from verts[i] to verts[i+1],
