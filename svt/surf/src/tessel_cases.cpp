...
{ 
    ...
    PNT		 cir_center; /* center of the circle of intersection */
    ...
    ALLOCN(comp_verts, PNT, num_int_verts+2);
    ...
    VEC3_V_OP_V(comp_verts[num_int_verts+1], edge[1]->int_pts[ipt[1]],+,atoms[atom_id].center);
...
{ 
    ...
    if (face_atoms[1] >= 0)
    {
        ...
        if (cir_radius > 0)
        {
            /* check to see if this face contains the center of the circle of intersection */
...
{ 
    ...
    if ((theta >= 1.0) && (theta <= 1.0 + GP_EPS))
        theta = 0;
    else if ((theta <= -1.0) && (theta >= -1.0-GP_EPS))
        theta = fPI;
    else
        theta = acos(theta);
...
{ 
    ...
    *radius = r*r - t*t*DOTPROD3(C1_C0, C1_C0);
    ...
    if (*radius < 0)
    {
        SURFOUT << "find_circle(): Negative circle radius: atoms (" << face_atoms[0] << " " << face_atoms[1] << ") radius " << *radius << endl;
        *radius = 0;
    }
    *radius = sqrt(*radius);
