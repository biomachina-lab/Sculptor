...
void gen_torus(VertexType **tor_pts, PNT* probe_centers, int num_verts, int flip, double radius, int cusp)
{
    ...
    j = 1;
    VEC3_ASN_OP(fcenter, =, probe_centers[0]);
    gen_linear_recurse(tor_pts[0][0],tor_pts[1][0],r[0],&j,fcenter,fradius,0);

    if (j > n)
    {
        SURFOUT << "gen_torus(): Too many torus verts " << j << " (limit " << n << ")" << endl;
        return ;
    }
    old_j = j;
    ...
    for(i = 1; i < num_verts; i++)
    {
        ...
        {
            if (flip)
                gen_tris(&r[l][m], &r[k][m+1], &r[k][m]);
            else
                gen_tris(&r[l][m], &r[k][m], &r[k][m+1]);

            if (((!cusp)&&(j <= old_j))||(m != j-2))
            {
                if (flip)
                    gen_tris(&r[l][m], &r[l][m+1], &r[k][m+1]);
                else
                    gen_tris(&r[l][m], &r[k][m+1], &r[l][m+1]);
            }
        }
    if (j < old_j)
    {
        ...
        {
            if (flip)
                gen_tris(&r[l][m], &r[l][m+1], &r[k][j-1]);
            else
                gen_tris(&r[l][m], &r[k][j-1], &r[l][m+1]);
        }
    }
