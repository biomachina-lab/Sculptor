...
void gen_concave(VertexType **tor_pts, VertexType center[2], int flip_verts, VertexType cusp_pts[2], PNT probe[2], int n, struct Torus *cur_torus, int torus_cusp)
{
    ...
    if ((cur_torus->end_atoms[0] == cur_torus->prev->end_atoms[1]) || (cur_torus->end_atoms[0] == cur_torus->prev->end_atoms[0]))
    {
        /* generate a concave triangle intersected with a plane */
        gen_cuspy_concave_tri(tor_pts[0][0],tor_pts[1][0],center[0], flip_verts, probe[0], dradius,&plane_eq);
        if (torus_cusp) /* generate the extra tri for 2-pt torus cusp */
            gen_cuspy_concave_tri(tor_pts[1][0],cusp_pts[0],center[0],flip_verts, probe[0], dradius,&plane_eq);
    }
    else
    {
        gen_concave_tri(tor_pts[0][0],tor_pts[1][0],center[0], flip_verts, probe[0], dradius);
        if (torus_cusp) /* generate the extra tri for 2-pt torus cusp */
    ...
    {
        /* generate a concave triangle intersected with a plane */
        gen_cuspy_concave_tri(tor_pts[1][n-1],tor_pts[0][n-1],center[1],flip_verts,probe[1], dradius,&plane_eq);
        if (torus_cusp) /* generate the extra tris for 2-pt torus cusp */
            gen_cuspy_concave_tri(cusp_pts[1],tor_pts[1][n-1],center[1],flip_verts, probe[1], dradius,&plane_eq);
    }
    else
    {
        gen_concave_tri(tor_pts[1][n-1],tor_pts[0][n-1],center[1],flip_verts,probe[1], dradius);
        if (torus_cusp) /* generate the extra tris for 2-pt torus cusp */
            gen_concave_tri(cusp_pts[1],tor_pts[1][n-1],center[1],flip_verts, probe[1], dradius);
    }
}
...
{ 
    ...
    if ((sq_side_len[0] < EPS) && (sq_side_len[1] < EPS) && (sq_side_len[2] < EPS))
        return;
