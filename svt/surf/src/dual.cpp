...
{ 
    ...
    do
    {
        for(i = 0; i < 3; i++)
        {
            ...
            if (temp_v->mark == !MARKED)/* generate the feasible cell face for this vertex*/
            {
                temp_v->mark = MARKED;
...
void alloc_fc_faces()
{
    ...
    for(i = 0; i < rfaces_count; i++)
        if(rfaces[i].num_verts == 0)
            SURFOUT << "Vert " << i << " has zero edges in convex hull" << endl;
...
void gen_fc_face(struct	tvertex	*ch_vert, struct tface *ch_face)
{
    ...
    else
    {
        ...
        {
            SURFOUT << "Error: gen_fc_face(): ch_vert != either edge endpt" << endl;
            return;
        }
    }
    ...
    do
    {
        ...
        else
        {
            ...
            {
                SURFOUT << "gen_fc_face(): temp_f != either edge adjface" << endl;
                return;
            }
        }
        ...
        else
        {
            ...
            {
                SURFOUT << "gen_fc_face(): ch_vert != either edge endpt" << endl;
                return;
            }
        }
...
void dualize_planes_to_points(Vector* plane, int num_planes)
{
    ...
    for(i = 0; i < num_planes; i++)
    {
        ...
        VEC3_V_OP_S(pNew->v, plane[i].coeff, /, plane[i].coeff[3]);

        /* add Emiris and Canny's perturbation here */
        for(perturbation = j = 1; j < 4; j++)
        {
            perturbation *= i;
            pNew->v[j] += EPSILON*(perturbation % PERT_Q);
        }
...
{ 
    ...
    for(i = 0; i < rvertices_count; i++)
        SURFOUT << "Cell vert[" << i << "]: " << rvertices[i].v[X] << " " << rvertices[i].v[Y] << " " << rvertices[i].v[Z] << endl;
}
