...
extern svt_container_surf* g_pSurf;
...
void gen_convex(PNT* comp_verts, VertexType* p, PNT* probe_centers, VertexType* q, int num_p, int num_q, int same_order, int flip, int atom_id, int full_torus)
{
    ...
    for(guess_num_sph_side = 0, i = -1; i < num_p; i++)
    {
        if (i == -1)
        {
            VEC3_V_OP_V(diff0, q[0].Coord, -, atoms[atom_id].center);
        }
        else
        {
            VEC3_V_OP_V(diff0, p[i].Coord, -, atoms[atom_id].center);
        }
        if (i == num_p-1)
        {
        ...
        {
            VEC3_V_OP_V(diff1, p[i+1].Coord, -, atoms[atom_id].center);
        }
        NORMALIZE3(diff0); NORMALIZE3(diff1);
        angle = DOTPROD3(diff0, diff1);
        if ((angle >= 1.0) && (angle <= 1.0 + GP_EPS))
            angle = 0;
        else if ((angle <= -1.0) && (angle >= -1.0-GP_EPS))
            angle = PI;
        else
            angle = acos(angle);
        guess_num_sph_side += 1+2*((int)(angle*atoms[atom_id].radius/Max_Tess_Len+FCEIL));
    }
    ...
    for(i = 0; i < num_q; i++)
    {
        ...
        /* memcpy(torus_side[i].pt, probe_centers[i], sizeof(PNT)); */
    ...
    {
        SURFOUT << "Error: Gen_convex(): Too many points on the sphere!" << endl;
        SURFOUT << "       ft " << full_torus << " num_sph_side " << num_sph_side << ", guess_num_sph_side " << guess_num_sph_side << " angle " << angle << endl;
        SURFOUT << "       center (" << atoms[atom_id].center[X] << " " << atoms[atom_id].center[Y] << " " << atoms[atom_id].center[Z] << ") radius:" << atoms[atom_id].radius << endl;
        free(sph_side);
        return;
    }
    ...
    if (full_torus)
        get_full_torus_mesh(sph_side, num_sph_side, torus_side, num_q, flip, atom_id);
    else
        get_partial_torus_mesh(sph_side, num_sph_side, torus_side, num_q, flip, atom_id);
    free(sph_side);
    free(torus_side);
    free(big_p);
}
...
{ 
    ...
    {
        if(valid_point&flags[0])
            find_mid_pt(&mid_pt[0],tri_pt[0],tri_pt[1],atom_id);
        if(valid_point&flags[1])
            find_mid_pt(&mid_pt[1],tri_pt[1],tri_pt[2],atom_id);
        if(valid_point&flags[2])
            find_mid_pt(&mid_pt[2],tri_pt[2],tri_pt[0],atom_id);
...
{ 
    ...
    else
    {
        ...
        if (t < 0)
        {
            SURFOUT << "Error: find_mid_pt(): t = " << t <<" < 0" << endl;
            return;
        }
        t = sqrt(t);
        t1 = -b + t; t2 = -b -t;
...
{
    if (Write_Option == 1 && g_pSurf != NULL)
    {
        Num_polys++;
        g_pSurf->addTriangle( *pt0, *pt1, *pt2, Current_atom );
    } else
...
void add_tri(VertexType* pt0, VertexType* pt1, VertexType* pt2, int atype)
{
    ...
    else if (Write_Option == 2) /* store the triangle */
    {
        ...
        {
            SURFOUT << "Error: add_tri(): Too many triangles -- Max_Tri (" << Max_Gp_Polys << ") exceeded" << endl;
            return;
        }
    }
    else /* just count it */
