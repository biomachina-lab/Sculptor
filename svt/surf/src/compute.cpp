...
void init_and_compute()
{
    ...
    if (Max_Tess_Len <= 0)
        Max_Tess_Len = MAX_TESS_LEN;
    SURFOUT << "Max edge length: " << Max_Tess_Len << endl;
    ...
    svt_exception::ui()->progressPopup("Computing surface...", 0);
    try
    {
        for(i = 0; i < Num_atoms; i++)
        {
            ...
            if (i%100==0)
            {
                int iPerc = (int)((((float)(i) / (float)(Num_atoms)) * 100.0));
                svt_exception::ui()->progress( iPerc);
            }
        }
    } catch (svt_userInterrupt&)
    {
        SURFOUT << "User interrupted the computation of the surface..." << endl;
    }
    svt_exception::ui()->progressPopdown();

    SURFOUT << "Total triangles: " << Num_polys << " - total constraints: " <<  Tot_cons << endl;
    SURFOUT << "Max neighbors per atom: " << Max_cons << " - average neighbors per atom: " << Tot_cons*1.0/Num_atoms << endl;
...
{ 
    ...
    for (i = 0; i < Num_atoms; i++)
    {
        for( j = 0; j < 3; j++)
        {
            ...
            if (Extents[j][MINC] > temp_min)
                Extents[j][MINC] = temp_min;
            if (Extents[j][MAXC] < temp_max)
                Extents[j][MAXC] = temp_max;
        }
        total_radius += atoms[i].radius;
        if (Max_Radius < atoms[i].radius)
            Max_Radius = atoms[i].radius;
    }
    ...
    for(j = 0; j < 3; j++)
        Num_Voxels[j] = (int)((Extents[j][MAXC] - Extents[j][MINC]) / Voxel_Side + FCEIL);
    ...
    for(i = 0; i < Num_atoms; i++)
    {
        ...
        if (++j > MAX_DENSITY)
        {
            SURFOUT << "Error: Atom density > MAX_DENSITY" << endl;
            i = Num_atoms;
        }
        else
    ...
    for(i = 0; i < Num_atoms; i++)
        atoms[i].tes_origin[0] = atoms[i].tes_origin[1] = atoms[i].tes_origin[2] = -1e+22;
}
...
{ 
    ...
    /* plane 0 */
    Bounding_Tetra[0].coeff[X] = Bounding_Tetra[0].coeff[Y] = ZERO;
    ...
    if (Checks_On)
    {
        for(i = 0; i < Num_atoms; i++)
        {
            ...
            {
                SURFOUT << "Bounding tetra intersects ext radius sph for atom " << i << endl;
            }
        }
    }
}
...
{ 
    ...
    if (!compute_neighbors(atom_id, &num_constraints))
        return;

    compute_planes(atom_id, num_constraints,constraints);

    Tot_cons += num_constraints;
    if (Max_cons < num_constraints)
        Max_cons = num_constraints;
    ...
    if (find_origin(atom_id, constraints, num_constraints))
    {
        ...
        if (rvertices_count - redges_count + rfaces_count != 2)
            SURFOUT << "Some convex hull bug for atom_id " << atom_id << "case" << endl;
                ...
                for(l = 0; l < Atom_Grid[temp_index[2]].num_neighbors; l++)
                {
                    ...
                    if ((dist <=  SQ(effective_radius) + GP_EPS) && (i != n))
                    {
                        ...
                        *num_cons += 1;
                        ...
                        if (++p > MAX_CONSTRAINT)
                        {
                            SURFOUT << "Error: Number of neighbors of atom > MAX_CONSTRAINT!" << endl;
                            return FALSE;
                        }
                    }
                }
...
int find_origin(int atom_id, Vector* cons, int num_cons)
{
    ...
    else /* check to see if the feasible region is non-null */
    {
        ...
        atoms[atom_id].boundary = linear_prog(3,obj, extents, cons, num_cons, &sol, sol_cons);
        if (atoms[atom_id].boundary)
        {
            ...
            if (Checks_On)
            {
                ...
                for(i = 0; i < num_cons; i++)
                {
                    ...
                    if (lambda > GP_EPS)
                    {
                        SURFOUT << "Precision warning(): Solution barely satisfied in atom_id " << atom_id << ", constraint " << i << " lambda " << lambda << endl;
                    }
                }
                ...
                for(i = 0; i < num_cons; i++)
                {
                    ...
                    {
                        origin_feasible = FALSE;
                        SURFOUT << "Origin outside region in atom " << atom_id << ", constraint " << i << " by " << lambda - cons[i].coeff[3] << endl;
                    }
                }
            }
        }
...
void find_sol_cons(Vector *cons, int num_cons, int sol_cons[3], Vector sol)
{
    ...
    if (min_lambda[2] > GP_EPS)
        SURFOUT << "Find_sol_cons() warning: sol is somewhat away from vertex " << min_lambda[2] << endl;
}
...
void find_tes_origin(int atom_id, Vector* cons)
{
    ...
    if (found_origin)
    {
        ...
        if (DOTPROD3(diff, diff) >= sq_radius)
            SURFOUT << "Find_tes_origin(): Bad tes_origin: atom " << atom_id << ": (" << temp_origin[0] << " " << temp_origin[1] << " " << temp_origin[2] << ")" << endl;
...
{ 
    ...
    for(i = 0; i < rfaces_count; i++)
    {
        ...
        if ((tempd <= 0) || (FP_EQ_EPS(tempd, 0, GP_EPS)))
            continue;
        else
        {
            result = FALSE;
