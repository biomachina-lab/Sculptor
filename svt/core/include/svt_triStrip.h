...
#ifndef SVT_TRI_STRIP
#define SVT_TRI_STRIP
...
/**
 * A class a draw a tringle strip with automatic normal generation.
...
private:
   ...
   * to GL.
   */
  inline Real32* normal(Real32* p1, Real32* p2);
...
public:
   ...
   * class constructor
   */
  svt_triStrip(bool bLighted=true) : m_bLighted(bLighted) {} 
  /**
   * class destructor
   */
  virtual ~svt_triStrip() {}
...
inline void svt_triStrip::begin()
{ 
...
inline void svt_triStrip::beginNormals(Real32 fScaleFactor)
{ 
  m_p0=m_vfVertexBuffer; m_p1=m_vfVertexBuffer+3; m_p2=m_vfVertexBuffer+6;
  m_iNoTriangles=0; m_fNormalScale=fScaleFactor;
  
  glBegin(GL_LINES);
}

inline void svt_triStrip::point(Real32 x, Real32 y, Real32 z)
{
  if (m_bLighted)
	{
	  Real32* p = m_vfVertexBuffer + 3*(m_iNoTriangles%3);
	  p[0]=x; p[1]=y; p[2]=z;	  

	  if (m_iNoTriangles>=2)
		{ 
		  if (m_iNoTriangles & 1)
			glNormal3fv(normal(m_p2,m_p1));
		  else
			glNormal3fv(normal(m_p1,m_p2));
		  if (m_iNoTriangles==2)
			{
			  glVertex3fv(m_p0);
			  glVertex3fv(m_p1);
...
inline void svt_triStrip::point(const Real32* ptr)
{
  if (m_bLighted)
	{
	  Real32* p = m_vfVertexBuffer + 3*(m_iNoTriangles%3);
...
inline void svt_triStrip::point(const svt_vector4<Real32>& v)
{ point(v.c_data()); }

inline void svt_triStrip::pointNormal(Real32 x, Real32 y, Real32 z)
{
  Real32* p = m_vfVertexBuffer + 3*(m_iNoTriangles%3);
  p[0]=x; p[1]=y; p[2]=z;	  
...
inline void svt_triStrip::pointNormal(const Real32* ptr)
{
  Real32* p = m_vfVertexBuffer + 3*(m_iNoTriangles%3);
  memcpy(p, ptr, 3*sizeof(Real32));
...
inline void svt_triStrip::pointNormal(const svt_vector4<Real32>& v)
{ pointNormal(v.c_data()); }

inline int svt_triStrip::end()
{ glEnd(); return m_iNoTriangles-2; }

inline int svt_triStrip::endNormals()
{ glEnd(); return m_iNoTriangles-2; }


inline Real32* svt_triStrip::normal(Real32* p1, Real32*p2) {
  m_vfNormal[0]=  (p1[1]-m_p0[1])*(p2[2]-m_p0[2])-(p1[2]-m_p0[2])*(p2[1]-m_p0[1]);
  m_vfNormal[1]= -(p1[0]-m_p0[0])*(p2[2]-m_p0[2])+(p1[2]-m_p0[2])*(p2[0]-m_p0[0]);
  m_vfNormal[2]=  (p1[0]-m_p0[0])*(p2[1]-m_p0[1])-(p1[1]-m_p0[1])*(p2[0]-m_p0[0]);
  return m_vfNormal;
}
