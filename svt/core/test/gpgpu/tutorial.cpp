...
char* shader = \
"void mrt(in float2 coords: WPOS,        "\
"         uniform samplerRECT input,     "\
"         out float output0 : COLOR0,    "\
"         out float output1 : COLOR1,    "\
"         out float output2 : COLOR2,    "\
"         out float output3 : COLOR3 ) { "\
"float value = texRECT(input, coords);   "\
"output0 = value;                        "\
"output1 = -1 * value;                   "\
"output2 = 100 * value;                  "\
"output3 = -100 * value; }               ";
...
int main(int argc, char **argv) {
    ...
    fragmentProgram = cgCreateProgram (cgContext, CG_SOURCE, shader, fragmentProfile, "mrt", NULL);
    cgGLLoadProgram (fragmentProgram);
    inputParam = cgGetNamedParameter (fragmentProgram, "input");
    // 
