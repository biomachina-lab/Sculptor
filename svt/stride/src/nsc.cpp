...
//------------ here included #define from nsc.cpp
#define TEST_NSC 0
...
#define ST_ERROR UPDATE_FL,error
...
void svt_stride::error(const char *fmt, ...) {
  va_list args;
  fprintf(stderr,
...
void svt_stride::warning(const char *fmt, ...) {
  va_list args;
  fprintf(stderr,
...
double svt_stride::safe_asin(double f) {
  if ( (fabs(f) < 1.00) ) return( asin(f) );
  if ( (fabs(f) - 1.00)  < DP_TOL ) return(HALFPI);
  else ST_ERROR("ASIN : invalid argument %f", f);
  return(0.0);
  }


void * svt_stride::mycalloc(const char * filename, const int linenr,
                size_t nelem, size_t elsize) {
  int * ip;
  ip = (int *) calloc(nelem, elsize);
  if(ip) return(ip);
  else ST_ERROR("CALLOC : failed in file %s at line %d", filename, linenr);
  return(void *)ip;
  }


void * svt_stride::myrealloc(const char * filename, const int linenr,
                 void * ptr, size_t size) {
  int * ip;
  ip = (int *) realloc(ptr, size);
  if(ip) return(ip);
  else ST_ERROR("REALLOC : failed in file %s at line %d", filename, linenr);
  return(void*)ip;
  }
...
void svt_stride::icosaeder_vertices(double *xus) {
...
void svt_stride::divarc(double x1, double y1, double z1,
  ...
  if (dd < DP_TOL) ST_ERROR("divarc: rotation axis of length %f", dd);

  d1 = x1*x1+y1*y1+z1*z1;
  if (d1 < 0.5) ST_ERROR("divarc: vector 1 of sq.length %f", d1);
  d2 = x2*x2+y2*y2+z2*z2;
  if (d2 < 0.5) ST_ERROR("divarc: vector 2 of sq.length %f", d2);
...
int svt_stride::ico_dot_arc(int densit) { /* densit...required dots per unit sphere */
...
/* calculate tessalation level */
  ...
  if (n_dot < densit) {
    ST_ERROR("ico_dot_arc: error in formula for tessalation level (%d->%d, %d)",
      tess, n_dot, densit);
    }
...
/* calculate tessalation of icosaeder edges */
    for (i=0; i<11; i++) {
      for (j=i+1; j<12; j++) {
        ...
        for (tl=1; tl<tess; tl++) {
          if (tn >= n_dot) { ST_ERROR("ico_dot: tn exceeds dimension of xus"); }
...
/* calculate tessalation of icosaeder faces */
    ...
    if (n_dot != tn) {
      ST_ERROR("ico_dot: n_dot(%d) and tn(%d) differ", n_dot, tn);
      }
    }		/* end of if (tess > 1) */
  return n_dot;
  }		/* end of routine ico_dot_arc */

int svt_stride::ico_dot_dod(int densit) { /* densit...required dots per unit sphere */
/* dot distribution on a unit sphere based on an icosaeder *
 * great circle average refining of icosahedral face       */
...
/* calculate tesselation level */
  ...
  if (n_dot < densit) {
    ST_ERROR("ico_dot_dod: error in formula for tessalation level (%d->%d, %d)",
      tess, n_dot, densit);
    }
...
/* calculate tessalation of mixed edges */
    for (i=0; i<31; i++) {
      ...
      for (j=j1; j<j2; j++) {
        ...
        for (tl=1; tl<tess; tl++) {
          if (tn >= n_dot) {
            ST_ERROR("ico_dot: tn exceeds dimension of xus");
            }
          divarc(xus[3*i], xus[1+3*i], xus[2+3*i],
                 xus[3*j], xus[1+3*j], xus[2+3*j],
            tl, tess, &xus[3*tn], &xus[1+3*tn], &xus[2+3*tn]);
...
int svt_stride::unsp_type(int densit) {
  int i1, i2;
  i1 = 1;
...
int svt_stride::make_unsp(int densit, int mode, int * num_dot, int cubus) {
            ...
            for (ix=0; ix<nxbox; ix++)
            {
                ...
                for (iat=iii1; iat<iii2; iat++)
                {
                    ...
                    if (nnei)
                    {
                        ...
                        for (l=0; l<n_dot; l++)
                        {
                            if (xus[3*l]*(wknb+last)->x + xus[1+3*l]*(wknb+last)->y + xus[2+3*l]*(wknb+last)->z <= (wknb+last)->dot)
                            {
                                for (j=0; j<nnei; j++)
                                {
                                    if (xus[3*l]*(wknb+j)->x+xus[1+3*l]*(wknb+j)->y + xus[2+3*l]*(wknb+j)->z > (wknb+j)->dot)
                                    {
                                        last = j; break;
                                    }
                                }
...
*/
  ...
  if (NSC(co, ra, NAT, 42,FLAG_VOLUME | FLAG_ATOM_AREA | FLAG_DOTS, &area,
      &at_area, &volume,
      &dots, &ndots)) ST_ERROR("error in NSC");
